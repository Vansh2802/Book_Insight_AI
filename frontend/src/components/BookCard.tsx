...
export function BookCard({ book }: { book: Book }) {
  return (
    <Link
      href={`/books/${book.id}`}
      className="group block flex min-h-[16rem] flex-col rounded-2xl border border-zinc-200 bg-white/90 p-5 shadow-sm transition duration-200 hover:-translate-y-1 hover:scale-[1.01] hover:shadow-lg"
    >
      <div className="flex flex-1 flex-col">
        ...
        {book.description ? (
          <p className="mt-3 text-sm leading-6 text-zinc-700 flex-1">
            {shortText(book.description)}
          </p>
        ) : (
          <p className="mt-3 text-sm text-zinc-500 flex-1">No description available.</p>
        )}
      </div>
